        ...
        if (detect_flag == 1) {
            ...
            if (mn_state == ESP_MN_STATE_DETECTED) {
                ...
                for (int i = 0; i < mn_result->num; i++) {
                    ...
                    // Send the command_id over UART
                    char command_id_str[32];  // Buffer for the command_id string
                    snprintf(command_id_str, sizeof(command_id_str), "command_id: %d\n", mn_result->command_id[i]);
                    uart_write_bytes(UART_PORT_NUM, command_id_str, strlen(command_id_str));

                    // Send the prob value over UART
                    char prob_str[32];  // Buffer for the prob value
                    snprintf(prob_str, sizeof(prob_str), "prob: %.6f\n", mn_result->prob[i]);
                    uart_write_bytes(UART_PORT_NUM, prob_str, strlen(prob_str));
                }
                printf("-----------listening-----------\n");
            }
